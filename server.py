...
Use kg_load_domain() for other domains. Call FIRST.
"""
...
def build_orchestration_data(root_id="wbs_root", max_depth=5):
    ...
    try:
        ...
        def _collect(node_id, depth):
            ...
            sprint = meta.get("sprint", "") if isinstance(meta, dict) else ""
            all_nodes.append({
        ...
        for n in all_nodes:
            ...
            entry = {"id": n["id"], "name": n["name"], "domain": n["domain"], "deps": deps, "sprint": n.get("sprint", "")}
        ...
        def _sprint_sort_key(x):
            """Sort by sprint phase: 'Phase 1A' -> (0, '1A'), empty -> (1, 'zzz')"""
            s = x.get("sprint", "")
            if s and s.startswith("Phase "):
                return (0, s[6:])
            elif s:
                return (0, s)
            return (1, "zzz")
        ...
        return {
            ...
            "domains": domains,
            "next_actions": {
                "ready": sorted(ready, key=_sprint_sort_key),
                "in_progress": sorted(in_progress, key=_sprint_sort_key), "blocked": sorted(blocked, key=_sprint_sort_key),
            },
...
async def _handle_wbs_app(request):
    html_path = os.path.join(os.path.dirname(__file__), "wbs.html")
    try:
